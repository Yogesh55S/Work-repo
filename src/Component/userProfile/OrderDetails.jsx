...
const OrderDetails = () => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
        try {
          ...
          const data = await fetchWithAuth(`${API_URL}/orders/details/${orderId}`, token);
  ...
  if (!orderDetails) {
    return <p className="p-4 text-center text-gray-600">Loading order details...</p>;
  }
    ...
    <div className="max-w-6xl mx-auto p-4 pt-28">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="border p-6 rounded-lg shadow-md">
          ...
          <p className="mb-2">
            <strong>Order Date:</strong> {new Date(orderDetails.createdAt).toLocaleDateString("en-GB", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })}
          ...
          <p className="mb-2">
            <strong>Delivery Date:</strong> {new Date(new Date(orderDetails.createdAt).getTime() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString("en-GB", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })}
        ...
        <div className="border p-6 rounded-lg shadow-md">
          ...
          <p className="mb-2">
            <strong>Mobile Number:</strong> {orderDetails.customerMobile || "N/A"}
          </p>
          <p className="mb-2">
            <strong>Address:</strong> {orderDetails.address ? (
              <span>
                {orderDetails.address.houseNumber}, {orderDetails.address.city}, {orderDetails.address.state} - {orderDetails.address.zip}, {orderDetails.address.country}
              </span>
            ) : (
              "N/A"
            )}
